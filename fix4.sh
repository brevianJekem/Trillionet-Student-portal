...
# 1. Add price + description columns to schema
python3 -c "
path = 'server/src/db/schema.sql'
with open(path) as f:
    content = f.read()
old = '''CREATE TABLE IF NOT EXISTS packages (
  id             TEXT PRIMARY KEY,
  name           TEXT NOT NULL,
...
with open(path, 'w') as f:
    f.write(content)
print('1/4 schema.sql updated')
"

# 2. Real catalog in seed.js
...
JSEOF
echo "2/4 seed.js — real catalog written"

# 3. Backend API returns price + description
python3 -c "
path = 'server/src/routes/packages.js'
with open(path) as f:
    content = f.read()
old = '''function toClientShape(row) {
  return {
    id: row.id,
    name: row.name,
...
new = '''function toClientShape(row) {
  return {
    id: row.id,
    name: row.name,
    category: row.category,
...
with open(path, 'w') as f:
    f.write(content)
print('3/4 packages.js route updated')
"

# 4. Frontend — show prices
python3 -c "
...
content = content.replace(
    ...
    '''<thead><tr><th>Package</th><th>Category</th><th>Instructor</th><th>Price</th><th></th></tr></thead>'''
)
content = content.replace(
    ...
    '''<td style={{ fontWeight: 500 }}>
                      {p.name}
                      {p.description && <div style={{ fontSize: 11.5, color: 'var(--text-secondary)', fontWeight: 400, marginTop: 2 }}>{p.description}</div>}
                    </td>
                    <td className=\"muted\">{p.category}</td>
                    <td className=\"muted\">{p.instructor}</td>
                    <td style={{ fontWeight: 500 }}>KSh {p.price.toLocaleString()}</td>
                    <td style={{ textAlign: 'right' }}>'''
)
content = content.replace(
    'colSpan={4}',
...
content = content.replace(
    ...
    '''<p className=\"sub\">{p.category} · {p.instructor} · KSh {p.price.toLocaleString()}</p>
                      {p.description && <p className=\"sub\" style={{ marginTop: 4 }}>{p.description}</p>}'''
)

with open(path, 'w') as f:
    f.write(content)
print('4/4 Packages.jsx — prices displayed')
"

echo ""
echo "All done. Verify with: grep 'price' src/pages/Packages.jsx server/src/routes/packages.js"